process_qvr_data <- function(the_data, id_column = "Appl.Id", hd_branch = TRUE) {
	the_data <- as.data.frame(the_data) ## to prevent problems with tibbles
	the_data <- unique(the_data)
	if (sum(duplicated(the_data[,id_column])) > 0) {
		split_data <- split(the_data, the_data[,id_column])
		split_data <- lapply(split_data, as.data.frame) ## necessary in case you're trying to use this on a tibble, because length(unique(split_data[[y]][,x])) where y is a tibble is always 1
	...
	if (all("Clinical.Trial" %in% colnames(the_data), "Phase.3.Trials" %in% colnames(the_data)) == TRUE) {
		the_data$is_trial <- sapply(1:nrow(the_data), function(x) ifelse(any(the_data$Clinical.Trial[x] %in% c("1", "1.0", "Y"), the_data$Phase.3.Trials[x] %in% c("1", "1.0", "Y")), "Y", "N"))  
	}
	...
	if (all(hd_branch, "PCC" %in% colnames(the_data)) == TRUE) {
